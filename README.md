...
ctx-profile watch --all-sessions
ctx-profile latest
...
- `c` clear the pinned session filter and show all sessions
...
When you select a session, the dashboard pins to that session and all views show only events from that session. Press `c` to clear the pinned session filter and show all-session aggregation. Toggle `a` to switch all-session mode, or cycle source with `s` to clear the pin and follow latest for that source.
...
/context-profiler watch --all-sessions
