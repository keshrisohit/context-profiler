...
class ContextProfilerApp(App):
    ...
    BINDINGS = [
        ...
        Binding("c", "clear_session_filter",   "Clear Session"),
    ...
    def __init__(self, pin_session=None, source_filter="all", all_sessions=False):
        super().__init__()
        ...
        self.all_sessions_mode = bool(all_sessions and not pin_session)
        self.session_query = ""
        self._session_rows = {}
        self._session_paths = []
        ...
        with TabbedContent(id="tabs"):
            ...
            with TabPane("Sessions", id="sessions"):
                yield Static(
                    ...
                    "Enter to pin. Press c to clear the session filter and show all sessions. "
                    "Use s to switch source.",
                    id="sessions-help",
                )
    ...
    def _update_status_bar(self):
        ...
        self.query_one("#status-bar").update(
            ...
            f"[c]clear-session [o]sort [m]group [i]ignored [s]source [q]quit[/]{bnr}"
        )
    ...
    def _update_overview(self):
        ...
        if self.all_sessions_mode:
            overview_scope = "[bold green]ALL SESSIONS[/] [dim](session filter cleared)[/]"
            session_filter_hint = "[green]No session filter is active.[/]"
        elif self.pinned_session:
            overview_scope = "[bold yellow]PINNED SESSION[/]"
            session_filter_hint = "[bold yellow]Session filter is active. Press c to clear it and show all sessions.[/]"
        else:
            overview_scope = "[cyan]FOLLOW LATEST SESSION[/]"
            session_filter_hint = "[dim]Following latest session. Press c to show all sessions.[/]"

        self.query_one("#overview-content").update(f"""
[bold]Current Session[/]
[dim]Scope:[/]         {overview_scope}
...
""")
    ...
    def action_clear_session_filter(self):
        self.pinned_session = None
        self.all_sessions_mode = True
        self._clear_session_search()
        self.banner = "session filter cleared: showing all sessions"
        self._refresh()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all-sessions", action="store_true",
                        help="Start with the session filter cleared and aggregate all sessions")
